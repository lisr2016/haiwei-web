  ...
  <div class="app-container">
    ...
    >
      ...
      <el-table-column fixed="right" label="操作">
        <template slot-scope="scope">
          <el-button type="text" size="small" @click="add('2', scope.row)">
            编辑
          </el-button>
          <el-button
            type="text"
  ...
  methods: {
    ...
    async handleDeleteOrg(row) {
      this.currentData = Object.assign({}, row, {
        levelText: this.levelValues[row.level - 1]
      });
      const params = {
      ...
      this.$message({
        message: `${row.isDeleted ? `恢复成功` : `注销成功`}`,
        type: "info"
      });
      this.fetchData();
    ...
    submit() {
      ...
      const params = this.isEdit
        ? Object.assign({}, this.form, { organizationId: this.organizationId })
        : this.form;
      // delete params.name;
      ...
      this.$refs.form.validate(async valid => {
        if (valid) {
          await api(params);
          this.$message({
            message: this.isEdit ? "编辑机构成功" : "新增机构成功",
            type: "success"
          });
          this.fetchData();
